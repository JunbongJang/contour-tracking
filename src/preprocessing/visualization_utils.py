...
def visualize_points(root_path, points_path, cmap, image_format, a_folder, save_path):
    ...
    images = glob(f"{root_path}/{a_folder}/img/*{image_format}")
    points_textfiles = glob(f"{root_path}/{points_path}/*.txt")
    assert len(images) == len(points_textfiles)
    ...
    for image_path in images:
        ...
        textfile_path = f"{points_path}/{image_name}.txt"
    ...
    ndarray_points = np.loadtxt(f"{points_path}/{last_image_name}.txt")
...
def visualize_points_in_POST(root_assets_path, root_generated_path):
    ...
    root_path = f'{root_assets_path}/PoST_gt'
    image_format = '.png'
    # all_folders = glob(f"{root_path}/*")
    ...
    for a_folder in all_folders:
        ...
        save_path = f"{root_generated_path}/PoST/{a_folder}/gt/"
        gt_points_path = f"{root_assets_path}/PoST_gt/{a_folder}/points/"
        visualize_points(root_path, gt_points_path, cmap, image_format, a_folder, save_path)

        # save_path = f"{root_generated_path}/PoST/{a_folder}/generated/"
        # predicted_points_path = f"{root_assets_path}/PoST_generated/{a_folder}/"
        # visualize_points(root_path, predicted_points_path, cmap, image_format, a_folder, save_path)
