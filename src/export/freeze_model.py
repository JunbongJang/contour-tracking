...
flags.DEFINE_integer('height', 480, 'Image height for training and evaluation.')
flags.DEFINE_integer('width', 384, 'Image height for training and evaluation.')
flags.DEFINE_integer('resize_to_height', 480, 'Height to resize to')
flags.DEFINE_integer('resize_to_width', 384, 'Width to resize to')
flags.DEFINE_bool(
    'output_row_col', False, 'Whether to output flow in row, col '
    'format. If False, will output in col, row format.')
flags.DEFINE_string('image1_path', 'uflow/assets/pc_cellflow/training/img/040119_PtK1_S01_01_phase_3_DMSO_nd_03/040119_PtK1_S01_01_phase_3_DMSO_nd_03_001.png',
                    'Path to image1 for quality control.')
flags.DEFINE_string('image2_path', 'uflow/assets/pc_cellflow/training/img/040119_PtK1_S01_01_phase_3_DMSO_nd_03/040119_PtK1_S01_01_phase_3_DMSO_nd_03_002.png',
                    'Path to image2 for quality control.')
flags.DEFINE_string('segmentation1_path', 'uflow/assets/pc_cellflow/training/segmentation/040119_PtK1_S01_01_phase_3_DMSO_nd_03/040119_PtK1_S01_01_phase_3_DMSO_nd_03_001.png',
                    'Path to segmentation1 for quality control.')
flags.DEFINE_string('segmentation2_path', 'uflow/assets/pc_cellflow/training/segmentation/040119_PtK1_S01_01_phase_3_DMSO_nd_03/040119_PtK1_S01_01_phase_3_DMSO_nd_03_002.png',
                    'Path to segmentation2 for quality control.')
flags.DEFINE_float(
...
def convert_graph(checkpoint_dir, height, width, output_row_col):
  ...
  segmentation1_np = cv2.imread(FLAGS.segmentation1_path, cv2.IMREAD_GRAYSCALE).astype('uint8') / 255.
  segmentation2_np = cv2.imread(FLAGS.segmentation2_path, cv2.IMREAD_GRAYSCALE).astype('uint8') / 255.
  segmentation1_np = cv2.resize(segmentation1_np, (FLAGS.width, FLAGS.height))
  segmentation2_np = cv2.resize(segmentation2_np, (FLAGS.width, FLAGS.height))
  segmentation1_np = np.expand_dims(segmentation1_np, axis=-1)
  segmentation2_np = np.expand_dims(segmentation2_np, axis=-1)

  # Compute test flow and save.
  flow_np = sess.run(flow, feed_dict={image1: image1_np, image2: image2_np}) # , segmentation1: segmentation1_np, segmentation2: segmentation2_np
