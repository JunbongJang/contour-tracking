...
class UflowTest(absltest.TestCase):
  ...
  def test_inference(self):
    ...
    segmentation1 = np.random.randn(256, 256, 1).astype('uint8')
    segmentation2 = np.random.randn(256, 256, 1).astype('uint8')
    uflow = ContourFlow()
    flow = uflow.infer(image1, image2, segmentation1, segmentation2)
  ...
  def test_train_step(self):
    ...
    ds = tf.data.Dataset.from_tensor_slices(
        (tf.zeros([1, 2, 256, 256, 3], dtype=tf.float32), { 'segmentations': tf.zeros([1, 2, 256, 256, 1], dtype=tf.uint8) } ) ).repeat().batch(1)
