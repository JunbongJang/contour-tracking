...
class CostEmbedding4d_64(tf.keras.layers.Layer):
...
class MaxPool4d(tf.keras.layers.Layer):
    def __init__(self, kernel_size, stride, padding, dim='support'):
        super().__init__()
        
        self.pool = tf.keras.layers.MaxPooling2D(kernel_size, stride, padding='valid')
        self.dim = dim
...
class Interpolate4d(tf.keras.layers.Layer):
...
class Conv4d(tf.keras.layers.Layer):
    def __init__(self,
            in_channels,
            out_channels,
            kernel_size,
