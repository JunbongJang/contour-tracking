...
def sample_points_for_implicit_cycle_consistency(source_sampled_contour_index, source_seg_points, target_seg_points, NUM_NEIGHB_SAMPLE_POINTS=20):
    ...
    # get 10 nearby points for each closest_target_sampled_seg_points
    ...
    nearby_target_contour_index_tensor = tf.reshape(nearby_target_contour_index_tensor, shape=[batch_size, -1])  # (8, 100, 1)
    
    return nearby_target_contour_index_tensor


def find_max_corr_contour_indices(occ_contour_forward, closest_cur_sampled_seg_points):
    ...
    max_indices_of_closest_cur_sampled_seg_points = tf.math.argmax(occ_contour_forward, axis=-1)
    max_indices_of_closest_cur_sampled_seg_points = tf.cast(max_indices_of_closest_cur_sampled_seg_points, dtype=tf.int32)
    max_indices_of_closest_cur_sampled_seg_points = tf.reshape(max_indices_of_closest_cur_sampled_seg_points, shape=[-1])
    batch_index_list = tf.repeat(tf.range(batch_size), repeats=NUM_SAMPLE_POINTS)
    indices = tf.stack([batch_index_list, max_indices_of_closest_cur_sampled_seg_points], axis=1)  # shape is (number of points, 2)

    predicted_cur_contour_indices = tf.gather_nd(closest_cur_sampled_seg_points, indices)
    predicted_cur_contour_indices = tf.reshape(predicted_cur_contour_indices, shape=[batch_size, -1])  # (8, 10, 1)

    return predicted_cur_contour_indices


def create_GT_occupnacy(sampled_contour_indices, seg_points_shape, sampled_seg_points):
    '''
        sampled_contour_indices shape: (8, 10)
        seg_points shape: (8, 1640)

        Return occupancy tensor with a shape [8, 10 (source points), 100 (target points)]  
    ...
    NUM_SAMPLE_POINTS = sampled_contour_indices.shape[1]  # 10
    batch_index_list = tf.repeat(tf.range(batch_size), repeats=NUM_SAMPLE_POINTS)
    ...
    # Return occupancy tensor with a shape (8, 1640) with value 1 for indices at sampled_contour_indices and 0 otherwise
    gt_occ = tf.tensor_scatter_nd_update(gt_occ, indices, updates)  # (8, 1640)
    
    # get gt_occ at sampled_seg_points
    NUM_SAMPLE_POINTS = sampled_seg_points.shape[1]  # 100
    batch_index_list = tf.repeat(tf.range(batch_size), repeats=NUM_SAMPLE_POINTS)
    sampled_seg_points = tf.reshape(sampled_seg_points, shape=[-1])
    indices = tf.stack([batch_index_list, sampled_seg_points], axis=1)  # shape is (number of points, 2)

    gt_occ = tf.gather_nd(gt_occ, indices)
    gt_occ = tf.reshape(gt_occ, shape=[batch_size, -1])  # (8, 100)
