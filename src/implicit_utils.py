...
def find_max_corr_contour_indices(occ_contour_forward, closest_cur_sampled_contour_indices):
    ...
    indices = tf.stack([batch_index_list, max_indices_of_closest_cur_sampled_contour_indices], axis=1)  # shape is (number of points, 2)

    predicted_cur_contour_indices = tf.gather_nd(closest_cur_sampled_contour_indices, indices)
    predicted_cur_contour_indices = tf.reshape(predicted_cur_contour_indices, shape=[batch_size, -1])  # (batch_size, 10)
...
def create_GT_occupnacy(sampled_contour_indices, seg_points_shape, sampled_nearby_contour_indices):
    '''
        sampled_contour_indices shape: (batch_size, 50) represent the initial points before cycle consistency
        seg_points_shape = (batch_size, 1640) tell the maximum number of points in a contour
        sampled_nearby_contour_indices shape: (batch_size, 1000)  <--- 1000 = 20 nearby points * 50 initial points 

        Return ground truth occupancy tensor with a shape [batch_size, 1000 (potential target points)]   
